 ...
 *
 * @note
 * 	This functions will start either i2c peripheral (I2C0, I2C1) depending on the TypeDef passed in.
 *
 * @param[in] i2c
...
void i2c_start(I2C_TypeDef *i2c, uint32_t device_address, uint32_t register_address, bool rw, uint16_t *rx_data, uint32_t event_cb){
	EFM_ASSERT((i2c->STATE&_I2C_STATE_STATE_MASK)==I2C_STATE_STATE_IDLE); // triggered of not done with previous I2C op
	sleep_block_mode(I2C_EM_BLOCK);

	i2c_cmd.current_state = SEND_START_CMD;
	i2c_cmd.i2c = i2c;
...
void i2c_ack(){
	switch(i2c_cmd.current_state){
		...
		case DATA_RECEIVE_MSB:
			EFM_ASSERT(false);
			break;

		case DATA_RECEIVE_LSB:
			EFM_ASSERT(false);
			break;
...
void i2c_rxdatav(){
	switch(i2c_cmd.current_state){
		...
		case DATA_RECEIVE_MSB:
			*(i2c_cmd.rx_data) <<= SHIFT;
			*(i2c_cmd.rx_data) |= i2c_cmd.i2c->RXDATA;
			i2c_cmd.current_state++;
